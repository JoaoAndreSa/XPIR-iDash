...
void NFLlib_old::setNewParameters(unsigned int polyDegree_, unsigned int aggregatedModulusBitsize_)
{
...
void NFLlib_old::setmodulus(uint64_t aggregatedModulusBitsize_)
{
  ...
  {
    std::cout << "NFLlib_old: CRITICAL. Modulus of " << aggregatedModulusBitsize_ 
...
void NFLlib_old::setpolyDegree(unsigned int polyDegree_)
{
  polyDegree = polyDegree_;
...
uint64_t* NFLlib_old::getmoduli() { return moduli; }
unsigned short NFLlib_old::getnbModuli() { return nbModuli; }
unsigned int NFLlib_old::getpolyDegree() { return polyDegree; }
void NFLlib_old::copymoduliProduct(mpz_t dest) { mpz_init_set(dest, moduliProduct); }
...
poly64 NFLlib_old::allocBoundedRandomPoly(uint64_t upperBound_, bool uniform_) 
{
...
void NFLlib_old::setBoundedRandomPoly(poly64 res, uint64_t upperBound_, bool uniform_) 
{
  poly64 rnd, rnd_orig;
  uint64_t mask;
  ...
  if (uniform_ == false){
    ...
    for (unsigned int cm = 0 ; cm < nbModuli ; cm++)
    {
      ...
      {
        std::cout << "NFLlib_old: upperBound is larger than the moduli in setBoundedRandomPoly.";
...
poly64 *NFLlib_old::deserializeDataNFL(unsigned char **inArrayOfBuffers, uint64_t nbrOfBuffers, uint64_t dataBitsizePerBuffer, unsigned bitsPerCoordinate, uint64_t &polyNumber) {
...
void NFLlib_old::serializeData64 (uint64_t* indata, unsigned char* outdata, unsigned int bitsPerChunk, uint64_t nb_of_uint64)
{
...
void NFLlib_old::serializeData32 (uint32_t* indata, unsigned char* outdata, unsigned int bitsPerChunk, uint64_t nb_of_uint32){
  unsigned char *tmppointer;
  uint64_t *pointer64;
  pointer64 = (uint64_t *) outdata;
  uint32_t bitswritten=0;
...
poly64 NFLlib_old::allocpoly(bool nullpoly)
{
...
mpz_t* NFLlib_old::poly2mpz(poly64 p)
{
  mpz_t* resultmpz;
  mpz_t* tmpzbuffer;
...
// Debug printing function
void NFLlib_old::print_poly64(poly64 p, unsigned int coeff_nbr)
{
 std::cout << "[";
 for (unsigned int i = 0 ; i < coeff_nbr ; i++)
 {
...
uint64_t* NFLlib_old::bitsplitter_backtoback_internal_test (unsigned char** inDataBuffers, uint64_t nbrOfBuffers, uint64_t bitsPerBuffer, unsigned int bitsPerChunk, uint64_t totalbitsread,uint64_t bitsread, uint64_t* pointer64, unsigned int bitsToRead)
{
...
inline void NFLlib_old::bs_loop (unsigned char** inDataBuffers, uint64_t nbrOfBuffers, uint64_t bitsPerBuffer, unsigned int bitsPerChunk, uint64_t *&tmpdata, uint64_t bufferIndex, uint64_t &bitsread, size_t &subchunkIndex)
{
...
inline void NFLlib_old::bs_finish(poly64 &outdata, uint64_t int_uint64PerChunk, uint64_t polyNumber, uint64_t* splitData, uint64_t nbrOfBuffers, uint64_t bitsPerBuffer, unsigned int bitsPerChunk)
{
...
uint64_t* NFLlib_old::bitsplitter (unsigned char** inDataBuffers, uint64_t nbrOfBuffers, uint64_t bitsPerBuffer, unsigned int bitsPerChunk)
{
...
void NFLlib_old::internalLongIntegersToCRT(uint64_t* tmpdata, poly64 outdata, uint64_t int_uint64PerChunk, uint64_t totalNbChunks) 
{
