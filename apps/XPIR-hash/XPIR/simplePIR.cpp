...
bool run(DBHandler *db, uint64_t chosen_element, PIRParameters params){
  ...
  ******************************************************************************/
  ...
  // Result is in ... result
  cout << "asdadasdasdad" << result << endl;
...
/******************************************************************************
  ...
  ******************************************************************************/
  ...
  std::cout << "params.alpha = x; params.d = xs; crypto_params = LWE:97:1024:60;" << std::endl; 
  std::cout << "======================================================================" << std::endl;
  DBDirectoryProcessor db7;
 // if (db6.getErrorStatus()==true){
  //  std::cout << "SimplePIR : Error with db directory skipping test ..." << std::endl << std::endl;
  //} else {
    database_size = db7.getDBSizeBits(); nb_files = db7.getNbStream(); 
    maxFileBytesize = database_size/nb_files;
    chosen_element = 8;
    params.alpha = 1; params.d = 2; params.n[0] = 520; params.n[1] = 520;
