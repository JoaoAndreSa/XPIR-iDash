...
#include "PIRReplyGeneratorNFLFV_internal.hpp"
#include "sys/time.h"
...
PIRReplyGeneratorNFLFV_internal::PIRReplyGeneratorNFLFV_internal():
  lwe(false),
  currentMaxNbPolys(0),
  queriesBuf(NULL),
 ...
 **/
PIRReplyGeneratorNFLFV_internal::PIRReplyGeneratorNFLFV_internal( PIRParameters& param, DBHandler* db):
  lwe(false),
  currentMaxNbPolys(0),
...
void PIRReplyGeneratorNFLFV_internal::importFakeData(uint64_t plaintext_nbr)
{
  uint64_t files_nbr = 1;
  for (unsigned int i = 0 ; i < pirParam.d ; i++) files_nbr *= pirParam.n[i];

  uint64_t plain_bytesize = cryptoMethod->getpolyDegree()*cryptoMethod->getnbModuli()*8;
 ...
 **/
void PIRReplyGeneratorNFLFV_internal::importDataNFL(uint64_t offset, uint64_t bytes_per_file)
{
  ...
  for (int i=0; i < nbruns; i++)
    ...
    {
      vtstart = vtstop;
      std::cout <<"PIRReplyGeneratorNFLFV_internal: Element " << i+1 << "/" << nbruns << " imported\r" << std::flush;
     wasVerbose = true;
     lastindex = i+1;
    }
#endif
  ...
  for (int i=0; i < nbruns; i++)
    ...
    if (wasVerbose && lastindex != nbFiles)  std::cout <<"PIRReplyGeneratorNFLFV_internal: Element " << nbruns << "/" << nbFiles/pirParam.alpha << " imported" << std::endl;
#endif
  ...
  /** FILE PADDING **/
	...
	free(rawBits);
	std::cout<<"PIRReplyGeneratorNFLFV_internal: Finished importing the database in " << omp_get_wtime() - start << " seconds" << std::endl;
}

#ifdef SNIFFER
imported_database_t PIRReplyGeneratorNFLFV_internal::generateReplyGeneric(bool keep_imported_data, bool import)
{
...
//  std::ifstream *is = dbhandler->openStream(0,0);
  ...
  double end = omp_get_wtime();
	std::cout<<"PIRReplyGeneratorNFLFV_internal: Finished processing the sniffed data in " << end - start << " seconds" << std::endl;
	std::cout<<"PIRReplyGeneratorNFLFV_internal: Processing throughput " << (double)chunkBytesize*8*iterations / ((end - start)*1000000000ULL) << " Gbps" << std::endl;
...
imported_database_t PIRReplyGeneratorNFLFV_internal::generateReplyGeneric(bool keep_imported_data, bool import)
{
  ...
  {
    std::cout << "PIRReplyGeneratorNFLFV_internal: Cannot handle aggregation or dimensions on databases requiring multiple iterations" << std::endl;
    std::cout << "PIRReplyGeneratorNFLFV_internal: Handling the database on a single iteration, this can cause memory issues ..." << std::endl;
    nbr_of_iterations = 1;
  ...
  {
    std::cout << "PIRReplyGeneratorNFLFV_internal: Database is considered too large, processing it in "
...
// #pragma omp parallel for
  ...
  {
    if (nbr_of_iterations > 1) cout << "PIRReplyGeneratorNFLFV_internal: Iteration " << iteration << endl;
	...
	std::cout<<"PIRReplyGeneratorNFLFV_internal: Total process time " << end - start << " seconds" << std::endl;
	std::cout<<"PIRReplyGeneratorNFLFV_internal: DB processing throughput " << 8*database_size/(end - start) << "bps" << std::endl;
	std::cout<<"PIRReplyGeneratorNFLFV_internal: Client cleartext reception throughput  " << 8*dbhandler->getmaxFileBytesize()/(end - start) << "bps" << std::endl;
  freeQueries();
...
void PIRReplyGeneratorNFLFV_internal::generateReplyGenericFromData(const imported_database_t database)
{
#ifndef TEST_NFL_PERF_ITERATIONS
  ...
  double end = omp_get_wtime();
	std::cout<<"PIRReplyGeneratorNFLFV_internal: Total process time " << end - start << " seconds" << std::endl;
	std::cout<<"PIRReplyGeneratorNFLFV_internal: DB processing throughput " << 8*dbhandler->getmaxFileBytesize()*dbhandler->getNbStream()/(end - start) << "bps" << std::endl;
	std::cout<<"PIRReplyGeneratorNFLFV_internal: Client cleartext reception throughput  " << 8*dbhandler->getmaxFileBytesize()/(end - start) << "bps" << std::endl;
  freeQueries();
}
...
// - database is small enough to be kept in memory
// - it has already been imported to it
void PIRReplyGeneratorNFLFV_internal::generateReplyExternal(imported_database_t* database)
{
  uint64_t max_readable_size, database_size, nbr_of_iterations;
 ...
 **/
void PIRReplyGeneratorNFLFV_internal::generateReply()
{
...
#ifdef DEBUG
  cout << "PIRReplyGeneratorNFLFV_internal:  currentMaxNbPolys = " << currentMaxNbPolys << endl;
#endif
    ...
    for (uint64_t j = 0 ; j < reply_elt_nbr ; j++) // Boucle de reply_elt_nbr PIR
    {
      ...
      {
        vtstart = vtstop;
        std::cout <<"PIRReplyGeneratorNFLFV_internal: Reply " << j+1 << "/" << reply_elt_nbr << " generated\r" << std::flush;
        wasVerbose = true;
      }
#endif
    ...
    if ( i > 0)
    {
      cout << "PIRReplyGeneratorNFLFV_internal: reply_elt_nbr_OLD: " << old_reply_elt_nbr << endl;
    }
#endif
    if (i > 0)
    {
  ...
  printf( "PIRReplyGeneratorNFLFV_internal: Global reply generation took %f (omp)seconds\n", omp_get_wtime() - start);
}


double PIRReplyGeneratorNFLFV_internal::generateReplySimulation(const PIRParameters& pir_params, uint64_t plaintext_nbr)
{
...
double PIRReplyGeneratorNFLFV_internal::precomputationSimulation(const PIRParameters& pir_params, uint64_t plaintext_nbr)
{
  setPirParams((PIRParameters&)pir_params);
  pushFakeQuery();
  importFakeData(plaintext_nbr);
  ...
  double result = omp_get_wtime() - start;
  std::cout << "PIRReplyGeneratorNFLFV_internal: Deserialize took " << result << " (omp)seconds" << std::endl;
  freeQueries();
  freeInputData();
  freeResult();
  delete dbhandler;
 ...
 **/
void PIRReplyGeneratorNFLFV_internal::generateReply(	lwe_query *queries_,
lwe_in_data* data,
unsigned int lvl,
lwe_cipher* result)
{
  ...
  for (unsigned int current_poly=0 ; current_poly < currentMaxNbPolys ; current_poly++)
    ...
    for (unsigned int offset = 0; offset < query_size; offset += 200)
    {
      ...
      {
        vtstart = vtstop;
        if(currentMaxNbPolys != 1) std::cout <<"PIRReplyGeneratorNFLFV_internal: Dealt with chunk " <<
...
lwe_in_data* PIRReplyGeneratorNFLFV_internal::fromResulttoInData(lwe_cipher** inter_reply, uint64_t reply_elt_nbr, unsigned int reply_rec_lvl)
{
...
//     	{
//	      // Ciphertexts can be serialized in a single block as a,b are allocatted contiguously
//	      tmp_in_data.p = cryptoMethod->deserializeDataNFLFV((unsigned char*)inter_reply[i][j].a, cryptoMethod->getPublicParameters().getCiphertextBitsize(), tmp_in_data.nbPolys);
 ...
 **/
unsigned long PIRReplyGeneratorNFLFV_internal::computeReplySizeInChunks(unsigned long int maxFileBytesize)
{
...
void PIRReplyGeneratorNFLFV_internal::initQueriesBuffer() {
...
void PIRReplyGeneratorNFLFV_internal::pushFakeQuery()
{
  char* query_element;
...
void PIRReplyGeneratorNFLFV_internal::pushQuery(char* rawQuery)
{
  ...
  {
    std::cout << "PIRReplyGeneratorNFLFV: Finished importing query (this message should appear only once)" << std::endl;
  }

}

void PIRReplyGeneratorNFLFV_internal::pushQuery(char* rawQuery, unsigned int size, int dim, int nbr)
{
...
size_t PIRReplyGeneratorNFLFV_internal::getTotalSystemMemory()
{
#ifdef __APPLE__
  int m[2];
  m[0] = CTL_HW;
...
void PIRReplyGeneratorNFLFV_internal::setPirParams(PIRParameters& param)
{
  freeQueries();
  freeQueriesBuffer();
  pirParam = param;
...
void PIRReplyGeneratorNFLFV_internal::setCryptoMethod(CryptographicSystem* cm)
{
...
{
#ifdef DEBUG
  std:cout << "PIRReplyGeneratorNFLFV_internal: freeing input_data" << std::endl;
#endif
  uint64_t theoretical_files_nbr = 1;
	for (unsigned int i = 0 ; i < pirParam.d ; i++) theoretical_files_nbr *= pirParam.n[i];
...
#ifdef DEBUG
  printf( "PIRReplyGeneratorNFLFV_internal: input_data freed\n");
#endif
}

void PIRReplyGeneratorNFLFV_internal::freeQueries()
{
...
void PIRReplyGeneratorNFLFV_internal::freeQueriesBuffer()
{
  if (queriesBuf != NULL){
    for (unsigned int i = 0; i < pirParam.d; i++){
...
void PIRReplyGeneratorNFLFV_internal::freeResult()
{
